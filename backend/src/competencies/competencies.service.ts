...
export class CompetenciesService {
  constructor(
    ...
    @InjectRepository(CompetencyRequirement)
    private requirementRepo: Repository<CompetencyRequirement>,
    @InjectRepository(EmployeeSkill)
    private skillRepo: Repository<EmployeeSkill>,
    @InjectRepository(TrainingProgram)
    private programRepo: Repository<TrainingProgram>,
  ...
  async deleteRequirement(jobRoleId: string, competencyId: string) {
    const result = await this.requirementRepo.delete({
      jobRoleId,
      competencyId,
    });
    if (result.affected === 0)
  ...
  async getGapAnalysis(employeeId: string, roleId: string) {
    ...
    const skillMap = new Map(
      skills.map((s) => [s.competencyId, s.currentLevel]),
    );
    ...
    return requirements.map((req) => {
      ...
      return {
        ...
        gap,
        status:
          gap <= 0 ? 'Meets' : gap === 1 ? 'Needs Improvement' : 'Critical',
      };
    });
  }
  ...
  async getDepartmentSummary() {
    ...
    employees.forEach((employee) => {
      if (!employee.jobRoleId) return; // Skip employees without assigned job roles
      ...
      if (summary) {
        const userReqs = allRequirements.filter(
          (r) => r.jobRoleId === employee.jobRoleId,
        );
        userReqs.forEach((req) => {
          ...
          const skill = allSkills.find(
            (s) =>
              ...
              s.competencyId === req.competencyId,
          );
    ...
    return Array.from(deptMap.entries())
      .filter(([_, scores]) => scores.required > 0) // Only return departments with requirements
  ...
  async getDepartmentUserSummary(department: string) {
    const employees = await this.employeeRepo.find({
      where: {
        department: department === 'Unassigned' ? IsNull() : department,
      },
      relations: ['jobRole'],
    });
    const allRequirements = await this.requirementRepo.find();
    ...
    return employees
      .filter((employee) => !!employee.jobRoleId) // Filter out employees without job roles
      .map((employee) => {
        let required = 0;
        let actual = 0;
        const userReqs = allRequirements.filter(
      ...
      employees.forEach((employee) => {
        if (employee.jobRoleId) {
          const req = allRequirements.find(
            (r) =>
              r.jobRoleId === employee.jobRoleId && r.competencyId === comp.id,
          );
          if (req) {
            const skill = allSkills.find(
