...
import { generateProcessDeviationPdf as generateDeviationPdf } from './generateProcessDeviationPdf';
...
export function ListTab() {
  ...
  useEffect(() => {
    if (id && deviations.length > 0) {
      ...
      if (targetDev && !isModalOpen) {
        ...
        navigate('/process-deviation', { replace: true });
      }
    }
  }, [id, deviations]);
  ...
  const fetchDeviations = async () => {
    try {
      const response = await api.get('/process-deviation');
